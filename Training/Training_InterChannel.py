...
def main(args):
    ...
    if Mic_conf == 'set0':
        ...
        model = InterConvTasnet_Icassp.ConvTasNet(num_sources=4 ,enc_kernel_size = 256, enc_num_feats = 512, msk_kernel_size = 3, msk_num_hidden_feats = 512, msk_num_layers = 5, msk_num_stacks = 3,NumberOfChannels = 5, MtoN_channels=  32, ico_channels = 32,  hiddenc_channels= 32).cuda()
        checkpoint = torch.load('./Models/'+str(32000)+'BestModel_InterChannel.pth.tar' , map_location=torch.device(device)   )
        model.load_state_dict(checkpoint)

    elif Mic_conf == 'set1':
        import InterConvTasnet_Icassp  
        model = InterConvTasnet_Icassp.ConvTasNet(num_sources=4 ,enc_kernel_size = 256, enc_num_feats = 512, msk_kernel_size = 3, msk_num_hidden_feats = 512, msk_num_layers = 5, msk_num_stacks = 3,NumberOfChannels = 5, MtoN_channels=  32, ico_channels = 32,  hiddenc_channels= 32).cuda()
        checkpoint = torch.load('./Models/'+str(32000)+'BestModel_InterChannel_set1.pth.tar' , map_location=torch.device(device)   )
        #model.load_state_dict(checkpoint)

    elif Mic_conf == 'set2':
        import InterConvTasnet_Icassp 
        model = InterConvTasnet_Icassp.ConvTasNet(num_sources=4 ,enc_kernel_size = 256, enc_num_feats = 512, msk_kernel_size = 3, msk_num_hidden_feats = 512, msk_num_layers = 5, msk_num_stacks = 3,NumberOfChannels = 5, MtoN_channels=  32, ico_channels = 32,  hiddenc_channels= 32).cuda()
        checkpoint = torch.load('./Models/'+str(32000)+'BestModel_InterChannel_set2.pth.tar' , map_location=torch.device(device)   )
       	model.load_state_dict(checkpoint)

     
    #checkpoint = torch.load('./'+str(16000)+'BestModel_InterChannel.pth.tar' , map_location=torch.device('cuda'))
    ...
    save_dir = "./ValidpklFiles_TASNet_"+Mic_conf+"/"
    best_loss = 1000
    n_epochs = 100
...
#            random.shuffle(List_RT60)
            ...
            with tqdm(range(0,NumberOfExamples, step_batch), unit="batch") as pbar:
                for ibatch in pbar:
                    ...
                    #Input =  torch.from_numpy(    np.concatenate((np.expand_dims(Mix[0:4,:],0), np.expand_dims(np.expand_dims(Mix[4,:],0), 1), np.expand_dims(np.expand_dims(Cotrib[4,4,:],0), 1) ), 1)         ).float().to(device)   
                        ...
                        if not torch.isnan(Input_).any().item() and  not torch.isnan(Output_SS_).any().item() :   
                            ...
                            if epoch >= 0:
                                ...
                                for il in range(4):
                                    ...
                                    loss2 =  loss2 - si_sdr( SS[:,il,:].unsqueeze(1), Output_SS_[:,il,:].unsqueeze(1)) 
                                loss2 = loss2/4 #0.5*(loss2[0] +loss2[1] + loss2[3]) +0.5*loss2[2]  #loss_func(Output_SS_[:,:,:], SS[:,:,:])
                                #loss1 = loss1/4
                                #print(loss2)
                                if not(epoch%5):
                                      pass  
                                      sdr, sir, sar, perm = fast_bss_eval.bss_eval_sources(Output_SS_[0,:,:].detach().cpu(), SS[0,:,:].detach().cpu())
                                      print(sdr)
                                #print(loss2)
                                loss1 = 0.5*(loss1[0] +loss1[1] + loss1[3]) +0.5*loss1[2]   # torch.mean(loss1) #(torch.mean(loss1) + loss2)/2
                                del Input_, Output_SS_
                                #print(loss2)
                                #print(loss1)
                                ...
                                loss = 0.5*loss1 + 0.5*loss2 #(loss1.cuda() + loss2.cuda())/2
            ...
            with torch.no_grad():
                with tqdm(range(0,int(len(DirVal)/4)), unit="batch") as pbar:
                    for il in pbar:
                        ...
                        loss2 =  loss2 / 4 #loss_func(Output_SS[:,:,:], SS[:,:,:])
                ...
                if loss_valid[epoch] < best_loss:
                    ...
                    file_path = os.path.join('./'+str(16000*2)+'BestModel_InterChannel_'+Mic_conf+'_x.pth.tar' )
